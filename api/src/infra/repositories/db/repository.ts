...
import type { AdResponse } from '../../controllers/http/showData/types/responses'
...
export const make = async (deps: DBRepositoryDeps): Promise<DBRepository> => {
  const { logger } = deps
  ...
  const storeAd = async (ad: StoreData): Promise<boolean | Error> => {
    ...
    try {
      ...
      await client.query(insertQuery, [ad.title, ad.type, ad.area, ad.amount, ad.description, ad.placeId])
      logger.info(`Ad inserted!`)
      return true
    } catch (error) {
      // handle error
